...
- `.portals/` (local sync metadata)
- `.venv/`, `venv/` (virtual environment)
- IDE settings (`.vscode/`, `.idea/`) - unless team agrees
- Log files (`*.log`)
- Temporary files (`*.tmp`, `*.swp`)

### Special case: `.portals/` metadata

**Decision for Portals**: ❌ **DO NOT commit** `.portals/` to Git
...
**What this means:**
- Each developer initializes their own `.portals/`
...
❌ **Bad**: Work for 3 days, make one huge commit
✅ **Good**: Make small commits every time you complete a piece
...
Then customize for your project (add `.portals/`, etc.)
...
├── feature/phase-1-local-adapter
├── feature/phase-2-notion-adapter
└── feature/phase-3-mirror-mode
```
...
4. ✅ **Use .gitignore**: Protect secrets, ignore generated files
5. ✅ **Never commit secrets**: Use `.env.example` pattern
6. ✅ **Keep main stable**: Use branches for experiments
7. ✅ **Test before merging**: Run `pytest` before merging to main
8. ✅ **Pull before push**: Get latest changes first
9. ✅ **Tag releases**: Mark milestones with tags
10. ✅ **Document in README**: Help others (and future you)
...
**Ask yourself**:
1. Is it source code I wrote? → ✅ Commit
2. Is it documentation? → ✅ Commit
3. Is it generated by a tool? → ❌ Don't commit
4. Does it contain secrets? → ❌ Don't commit
5. Is it specific to my machine? → ❌ Don't commit
